...
class Model(BaseImputeModel):
    def __init__(self, exp_args:ExperimentArgs):
        ...
        self.T = self.exp_args['t']
        self.beta_0 = self.exp_args['beta_0']
        self.beta_T = self.exp_args['beta_t']
        self.in_channels = len(self.exp_args['targets'])
        self.out_channels = len(self.exp_args['targets'])
        self.res_channels = self.exp_args['res_channels']
        self.skip_channels = self.exp_args['skip_channels']
        self.num_res_layers = self.exp_args['num_res_layers']
        self.diffusion_step_embed_dim_in = self.exp_args['diffusion_step_embed_dim_in']
        self.diffusion_step_embed_dim_mid = self.exp_args['diffusion_step_embed_dim_mid']
        self.diffusion_step_embed_dim_out = self.exp_args['diffusion_step_embed_dim_out']
        self.s4_lmax = self.exp_args['s4_lmax']
        self.s4_d_state = self.exp_args['s4_d_state']
        self.s4_dropout = self.exp_args['s4_dropout']
        self.s4_bidirectional = self.exp_args['s4_bidirectional']
        self.s4_layernorm = self.exp_args['s4_layernorm']
        ...
        self.model = SSSDS4Imputer(
            ...
            self.diffusion_step_embed_dim_mid,
            self.diffusion_step_embed_dim_out,
            self.s4_lmax,
            self.s4_d_state,
            self.s4_dropout,
            self.s4_bidirectional,
            self.s4_layernorm
        )
    ...
    def _get_input(self, batch:dict) -> dict:
        ...
        _dh = self.calc_diffusion_hyperparams(self.T, self.beta_0, self.beta_T)

        audio = batch['observed_data'].permute(0, 2, 1)
        cond = batch['observed_data'].permute(0, 2, 1)
        mask = batch['ground_truth_mask'].permute(0, 2, 1)
        B, C, L = audio.shape
        device = audio.device
        ...
        res = {
            ...
            'conditional' : cond,
            'mask' : mask,
            'diffusion_steps' : diffusion_steps.view(B, 1)
        }
        return res
    ...
    def evaluate(self, batch:dict, training:bool) -> torch.Tensor:
        ...
        loss = calc_mae(res, observed_data, loss_mask)
        return loss
    ...
    def impute(self, batch:dict) -> torch.Tensor:
        ...
        logger.info('begin sampling, total number of reverse steps = %s' % T)
        imputed_samples = torch.zeros(self.n_samples, B, D, L).to(device)
        ...
        with torch.no_grad():
            for _ in range(self.n_samples):
                ...
                for t in range(T - 1, -1, -1):
                    ...
                    epsilon_theta = self.model.forward(inputs)
                    # update x_{t-1} to \mu_\theta(x_t)
                    x = (x - (1 - Alpha[t]) / torch.sqrt(1 - Alpha_bar[t]) * epsilon_theta) / torch.sqrt(Alpha[t])
                    if t > 0:
                        x = x + Sigma[t] * std_normal(size)  # add the variance term to x_{t-1}
            imputed_samples[_] = x
            imputed_samples = imputed_samples.permute(1, 0, 3, 2)
...
class S4Layer(nn.Module):
    ...
    def __init__(self, features, lmax, N=64, dropout=0.0, bidirectional=True, layer_norm=True):
        ...
        self.norm_layer = nn.LayerNorm(
            features) if layer_norm else nn.Identity()
...
class Residual_block(nn.Module):
    def __init__(self, res_channels, skip_channels,
        ...
        self.conv_layer = Conv(self.res_channels, 2 *
                               self.res_channels, kernel_size=3)
...
class Residual_group(nn.Module):
    def __init__(self, res_channels, skip_channels, num_res_layers,
        ...
        self.fc_t1 = nn.Linear(diffusion_step_embed_dim_in,
                               diffusion_step_embed_dim_mid)
        self.fc_t2 = nn.Linear(diffusion_step_embed_dim_mid,
                               diffusion_step_embed_dim_out)
...
class SSSDS4Imputer(nn.Module):
    ...
    def forward(self, input_data):
        ...
        conditional = input_data['conditional']
        mask = input_data['mask']
        diffusion_steps = input_data['diffusion_steps']
...
class SSKernelNPLR(nn.Module):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        if has_cauchy_extension and z.dtype == torch.cfloat:
            pass
            # Jelly: supress because python package not found.
    ...
    def _step_state_linear(self, u=None, state=None):
        ...
        if state.size(-1) == self.N:
            ...
            def contract_fn(p, x, y): return contract('r h n, r h m, ... h m -> ... h n',
                                                      # inner outer product
                                                      _conj(p), _conj(x), _conj(y))[..., :self.N]
        else:
