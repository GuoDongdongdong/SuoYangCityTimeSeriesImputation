...
class Model(BaseImputeModel):
    def __init__(self, exp_args:ExperimentArgs):
        ...
        self.input_dim = len(self.exp_args['targets'])
        self.time_length = self.exp_args['lookback_length']
        self.latent_dim = self.exp_args['latent_dim']
        self.encoder_sizes = (self.exp_args['encoder_sizes0'], self.exp_args['encoder_sizes1'])
        self.decoder_sizes = (self.exp_args['decoder_sizes0'], self.exp_args['decoder_sizes1'])
        self.beta = self.exp_args['beta']
        self.M = self.exp_args['m']
        self.K = self.exp_args['k']
        self.kernel = self.exp_args['kernel']
        self.sigma = self.exp_args['sigma']
        self.length_scale = self.exp_args['length_scale']
        self.kernel_scales = self.exp_args['kernel_scales']
        self.window_size = self.exp_args['window_size']
        self.n_samlpes = self.exp_args['n_samples']
        self.model = _GPVAE(
            self.input_dim,
            self.time_length,
            self.latent_dim,
    ...
    def impute(self, batch:dict) -> torch.Tensor:
        inputs = self._get_input(batch)
        res = self.model.forward(inputs, False, self.n_samlpes)
